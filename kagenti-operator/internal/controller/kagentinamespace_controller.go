...
// ensureRBAC creates a Role and RoleBinding for the backend to access the new namespace.
func (r *KagentiNamespaceReconciler) ensureRBAC(ctx context.Context, kns *agentv1alpha1.KagentiNamespace, targetNS string) error {
	roleName := "kagenti-backend-access"

	// Create a namespace-scoped Role with only the permissions the backend needs
	role := &rbacv1.Role{}
	roleKey := types.NamespacedName{Namespace: targetNS, Name: roleName}
	if err := r.Get(ctx, roleKey, role); err != nil {
		...
		role = &rbacv1.Role{
			...
			Rules: []rbacv1.PolicyRule{
				...
				{
					APIGroups: []string{"agent.kagenti.dev"},
					Resources: []string{"agents", "agentcards", "agentbuilds"},
	...
	// Create the RoleBinding referencing our custom Role
	rb := &rbacv1.RoleBinding{}
	rbKey := types.NamespacedName{Namespace: targetNS, Name: roleName}
